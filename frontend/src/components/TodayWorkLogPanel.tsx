...
import { TAG_COLORS, TAG_BG, TAG_TEXT, TAG_ORDER } from '../constants/tagColors'
...
export function TodayWorkLogPanel() {
  ...
  const heading   = isDark ? 'text-stone-500'             : 'text-stone-400'
  ...
  return (
    <div className={`${bg} rounded-xl border ${border} p-5`}>
      ...
      {isLoading ? (
        <p className={`text-sm ${emptyText} text-center py-6`}>Loading…</p>
      ) : total === 0 ? (
