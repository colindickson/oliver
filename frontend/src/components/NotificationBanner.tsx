  ...
  return (
    <div className="fixed bottom-4 right-4 flex flex-col gap-3 z-50 max-w-sm">
      {dueReminders.map(r => (
        <div
          key={r.id}
          className="bg-white border border-amber-200 shadow-soft-lg rounded-2xl p-4 flex items-start gap-3 animate-slide-up"
        >
          {/* Bell icon */}
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-stone-700 mt-1">{r.message}</p>
          </div>

          <button
            type="button"
            onClick={() => markDelivered.mutate(r.id)}
            className="w-6 h-6 flex items-center justify-center text-stone-300 hover:text-stone-500 hover:bg-stone-100 rounded-lg transition-colors flex-shrink-0"
